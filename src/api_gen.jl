...
MsgPack.pack(s, o::NvimApiObject{N}) where {N} = MsgPack.pack(s, Ext(N, o.hnd))
...
Bytes = Union{ByteString, Vector{UInt8}}
...
retconvert(typ::Type{Vector{T}}, c, val::Vector) where {T} = [retconvert(T,c,v) for v in val]
...
retconvert(::Type{Tuple{T,U}}, c, val::Vector) where {T,U} = (retconvert(T,c,val[1]),retconvert(U,c,val[2]))

retconvert(typ::Type{NvimApiObject{N}}, c, val::Ext) where {N} = NvimApiObject(c, val)::NvimApiObject{N}
